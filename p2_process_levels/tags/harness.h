...
static inline int *retrieve_set_alloc_params(int level, int new_alloc) {
  int *set_params = malloc( sizeof(int) * 4);

  set_params[0] = SYS_set_level_alloc;
  set_params[1] = 2;
  set_params[2] = level;
  set_params[3] = new_alloc;

  return set_params;
}

static inline int *retrieve_get_alloc_params(int level) {
  int *get_params = malloc( sizeof(int) * 3);

  get_params[0] = SYS_get_level_alloc;
  get_params[1] = 1;
  get_params[2] = level;
...
static inline int interpret_set_alloc_result(int ret_value) {
  if (ret_value < 0)
    return -1;
...
static inline int interpret_get_alloc_result(int ret_value) {
  if (ret_value < 0)
    return -1;
